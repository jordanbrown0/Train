...
function straight(length) = {(
    ...
    rotate = 0;
    length = length;
    point = function (n) n > length ? undef : [0, n];
)};
...
truck = {(
    height = 1;
    box = [ width, height*0.9, height*0.75 ];
    for (y=[-height, height]) {
        translate([0,y,height/2]) {
            wheelPair(height);
        }
    }
    translate([-box.x/2, -box.y/2, height-box.z])
...
function steamLoco(color) =
    let(boilerWidth = 3.05, boilerLength=10, cabWidth=4, cabLength=3, cabHeight=3.05)
    {(
        length = boilerLength+cabLength;
...
function boxcar(color) =
    ...
    {(
        ...
        color(color) translate([0,0,truck.height]) {
...
function caboose(color) =
    let(width = 3.05, height=3.05)
    let(topLength = 2, topHeight = 1)
    {(
    length = 10;
